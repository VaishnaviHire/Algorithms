...
class Solution:
    ...
    def cloneGraph(self, node):
        if not node:
            return None
        graph_clones = {}
        return self.cloneGraphNodes(node,graph_clones)

    def cloneGraphNodes(self,node,graph_clones):

        if graph_clones.has_key(node):
            return graph_clones[node]
        else:
            graph_clones[node] = UndirectedGraphNode(node.label)
            clone_node = graph_clones[node]
            for n in node.neighbors:
                clone_node.neighbors.append(self.cloneGraphNodes(n,graph_clones))
        return clone_node
